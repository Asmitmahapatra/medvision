...
import { 
  ...
  Smartphone, BarChart3, Lock, Sparkles, LineChart, Briefcase, LayoutDashboard, 
  Settings, ArrowUpRight, Star, Mic2
} from 'lucide-react';
...
const Dashboard = ({ copy }) => {
  ...
  return (
    <main className="relative min-h-screen overflow-x-hidden bg-gradient-to-b from-indigo-50 via-violet-50/20 to-slate-50">
      ...
      {/* Interactive Portal Templates Section */}
      <section className="relative px-4 py-24 sm:px-6 lg:px-8 xl:px-12 bg-gradient-to-br from-indigo-50/30 via-violet-50/20 to-pink-50/20">
        <div className="mx-auto max-w-7xl">
          <div className="mb-20 text-center">
            <div className="inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-indigo-100/60 via-violet-100/60 to-pink-100/60 px-5 py-2.5 mb-6 backdrop-blur-md border border-indigo-200/50">
              ...
              <span className="text-sm font-bold text-indigo-800">PORTAL SOLUTIONS</span>
            </div>
            ...
            <p className="text-lg text-slate-600 max-w-2xl mx-auto">Choose your role and access a tailored experience designed for your needs</p>
          </div>
            ...
            {[
              {
                ...
                description: 'Manage appointments, medical records, and symptoms in one place',
                features: ['Book appointments', 'View medical history', 'AI symptom checker', 'Multilingual support'],
                gradient: 'from-pink-600 to-rose-700',
                bgGradient: 'from-pink-50/60 to-rose-50/60',
                stats: { users: '2500+', satisfaction: '98%' },
                cta: 'Access Portal',
                path: '/patient',
                badge: 'Most Popular'
              },
              {
                ...
                icon: Stethoscope,
                title: 'Doctor Portal',
                description: 'Manage patient cases, appointments, and medical records efficiently',
                features: ['Patient dashboard', 'Appointment scheduling', 'Digital prescriptions', 'Patient communication'],
                gradient: 'from-indigo-600 to-blue-700',
                bgGradient: 'from-indigo-50/60 to-blue-50/60',
                stats: { providers: '500+', cases: '5000+' },
                cta: 'Doctor Access',
                path: '/doctor-signup',
              ...
              },
              {
                ...
                description: 'Get instant health guidance powered by advanced AI technology',
                features: ['24/7 availability', 'Instant responses', 'Hindi & English', 'No appointment needed'],
                gradient: 'from-violet-600 to-purple-700',
                bgGradient: 'from-violet-50/60 to-purple-50/60',
                stats: { conversations: '10K+', avgWait: '<30s' },
                cta: 'Chat Now',
                path: '/chatbot',
              ...
              },
              {
                ...
                description: 'Find and browse certified medical professionals with verified credentials',
                features: ['Verified credentials', 'Patient reviews', 'Multiple specialties', 'Book appointments'],
                gradient: 'from-emerald-600 to-teal-700',
                bgGradient: 'from-emerald-50/60 to-teal-50/60',
                stats: { doctors: '500+', specialties: '25+' },
                cta: 'Browse Doctors',
              ...
              return (
                <div
                  key={portal.id}
                  className="group relative rounded-2xl border border-gray-200/50 overflow-hidden transition duration-500 hover:shadow-2xl animate-float-up"
                  style={{ animationDelay: `${idx * 100}ms` }}
                >
                  {/* Background gradient overlay */}
                  <div className={`absolute inset-0 bg-gradient-to-br ${portal.bgGradient} opacity-0 group-hover:opacity-100 transition duration-500`} />
                  <div className="absolute inset-0 bg-white/80 backdrop-blur-sm" />
                  
                  {/* Animated background elements */}
                  <div className={`absolute -top-20 -right-20 h-40 w-40 rounded-full bg-gradient-to-br ${portal.gradient} opacity-10 group-hover:opacity-20 transition duration-500 blur-2xl`} />
                  
                  <div className="relative p-8 h-full flex flex-col">
                    {/* Badge */}
                    <div className="mb-4">
                      <span className={`inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r ${portal.gradient} px-4 py-1.5 text-xs font-bold text-white shadow-lg`}>
                    ...
                    {/* Icon and Title */}
                    <div className="mb-6">
                      <div className={`inline-flex items-center justify-center rounded-xl bg-gradient-to-br ${portal.gradient} p-4 mb-4 group-hover:scale-110 transition duration-300 shadow-lg`}>
                        <PortalIcon size={32} className="text-white" />
                      </div>
                      <h3 className="text-2xl font-bold text-slate-900 mb-2">{portal.title}</h3>
                      <p className="text-slate-600 text-sm leading-relaxed">{portal.description}</p>
                    </div>
                      ...
                      <ul className="space-y-2">
                        {portal.features.map((feature, i) => (
                          <li key={i} className="flex items-center gap-2 text-sm text-slate-700">
                            <div className={`h-1.5 w-1.5 rounded-full bg-gradient-to-br ${portal.gradient}`} />
                            {feature}
                          </li>
                        ))}
                      </ul>
                    ...
                    {/* Stats */}
                    <div className="mb-6 grid grid-cols-2 gap-4 p-4 rounded-lg bg-white/60 backdrop-blur-sm border border-gray-100/50">
                      {Object.entries(portal.stats).map(([key, value]) => (
                        <div key={key}>
                          <p className={`text-lg font-bold bg-gradient-to-r ${portal.gradient} bg-clip-text text-transparent`}>{value}</p>
                    ...
                    <button
                      ...
                      className={`relative group/btn overflow-hidden rounded-xl bg-gradient-to-r ${portal.gradient} px-6 py-3 font-semibold text-white shadow-lg transition duration-300 hover:shadow-xl hover:scale-105 flex items-center justify-center gap-2 w-full`}
                    >
                      <span className="absolute inset-0 bg-white/20 opacity-0 group-hover/btn:opacity-100 transition duration-300" />
                      <span className="relative flex items-center gap-2">
                        {portal.cta}
                        <ArrowUpRight size={16} className="transition duration-300 group-hover/btn:translate-x-0.5 group-hover/btn:-translate-y-0.5" />
                      </span>
                    </button>
                  </div>

                  {/* Hover line effect */}
                  <div className={`absolute bottom-0 left-0 h-1 w-full bg-gradient-to-r ${portal.gradient} scale-x-0 group-hover:scale-x-100 transition duration-500 origin-left`} />
                </div>
              );
            })}
          </div>
          ...
          <div className="rounded-2xl border border-indigo-200/50 bg-gradient-to-br from-white/80 to-indigo-50/40 backdrop-blur-md p-12 overflow-hidden relative">
            <div className="absolute inset-0 bg-gradient-to-br from-indigo-100/20 via-violet-100/10 to-transparent" />
            <div className="relative">
              <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-4">
                {[
                  { icon: Zap, label: 'Lightning Fast', value: '<2s Response' },
                  { icon: Shield, label: 'Secure & Private', value: 'HIPAA Compliant' },
                  { icon: Smartphone, label: 'Mobile First', value: 'Works Everywhere' },
                  { icon: Users, label: 'Community Driven', value: '2500+ Active Users' }
                ].map((item, idx) => {
                  const ItemIcon = item.icon;
                  return (
                    <div key={idx} className="text-center group animate-float-up" style={{ animationDelay: `${idx * 100}ms` }}>
                      <div className="inline-flex items-center justify-center rounded-xl bg-gradient-to-br from-indigo-100 to-violet-100 p-4 mb-4 group-hover:scale-110 transition duration-300">
                        <ItemIcon size={28} className="text-indigo-600" />
                      </div>
