  ...
  return (
    <main className="bg-gradient-to-b from-slate-50 via-indigo-50/20 to-slate-50 px-4 py-6 md:px-8 md:py-8 min-h-screen">
      <div className="mx-auto w-full max-w-6xl space-y-6">
        <BackButton to="/" label={copy.backToHome || 'Back to Home'} copy={copy} />
        {/* Header */}
        <section 
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex-1">
              <p className="inline-flex rounded-full bg-indigo-900/40 px-3 py-1 text-xs font-bold uppercase tracking-wider">
                {copy.healthManagement || 'Health Management'}
              </p>
              <h2 className="mt-3 text-4xl md:text-5xl font-black leading-tight">{copy.yourHealthRecords || 'Your Health Records'}</h2>
              <p className="mt-2 max-w-3xl text-sm text-indigo-100 md:text-base">
                {copy.manageAppointmentsRecords || 'Manage your appointments and medical records in one convenient place'}
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3 min-w-max">
              <div className="rounded-xl bg-white/10 backdrop-blur-sm border border-white/20 px-4 py-2 flex items-center gap-2">
                ...
                <div>
                  <p className="text-xs text-indigo-100 font-semibold">{copy.appointmentsTitle || 'Appointments'}</p>
