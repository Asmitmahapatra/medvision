...
const SignUp = ({ language = 'en', onLogin }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.parentName.trim()) {
      newErrors.parentName = signupCopy.parentNameRequired;
    }

    if (!formData.childName.trim()) {
      newErrors.childName = signupCopy.childNameRequired;
    }
  ...
  const handleSignUp = async (event) => {
    ...
    try {
      ...
      const localFallbackUser = {
        ...
        displayName: formData.parentName.trim(),
      ...
      <div className="mx-auto grid w-full max-w-6xl gap-6 md:grid-cols-5">
        ...
        <section className="rounded-3xl border border-indigo-100/60 bg-white/95 p-6 shadow-xl md:col-span-3">
          ...
          <form onSubmit={handleSignUp} className="mt-5 grid gap-4 md:grid-cols-2">
            <div>
              <label className="mb-1 block text-sm font-semibold text-gray-700">{signupCopy.parentNameLabel}</label>
              <div className="relative">
                <User className="absolute left-3 top-3.5 h-5 w-5 text-indigo-400" />
                <input
                  type="text"
                  name="parentName"
                  placeholder={signupCopy.parentNamePlaceholder}
            ...
            <div>
              <label className="mb-1 block text-sm font-semibold text-gray-700">{signupCopy.childNameLabel}</label>
              <div className="relative">
                <Baby className="absolute left-3 top-3.5 h-5 w-5 text-indigo-400" />
                <input
                  type="text"
                  name="childName"
                  placeholder={signupCopy.childNamePlaceholder}
                  value={formData.childName}
                  onChange={handleChange}
                  className={`w-full rounded-xl border py-2 pl-10 pr-4 outline-none transition focus:border-transparent focus:ring-2 focus:ring-indigo-500 ${
                    errors.childName ? 'border-red-500' : 'border-slate-300'
                  }`}
                />
              </div>
            ...
            <div>
              <label className="mb-1 block text-sm font-semibold text-gray-700">{signupCopy.ageLabel}</label>
              <input
                type="number"
                name="age"
                placeholder={signupCopy.agePlaceholder}
                value={formData.age}
                onChange={handleChange}
            ...
            <div>
              <label className="mb-1 block text-sm font-semibold text-gray-700">{signupCopy.phoneLabel}</label>
              <div className="relative">
                <Phone className="absolute left-3 top-3.5 h-5 w-5 text-indigo-400" />
                <input
                  type="tel"
                  name="phone"
                  placeholder={signupCopy.phonePlaceholder}
            ...
            <div className="md:col-span-2">
              <label className="mb-1 block text-sm font-semibold text-gray-700">{signupCopy.villageLabel}</label>
              <div className="relative">
                <MapPin className="absolute left-3 top-3.5 h-5 w-5 text-indigo-400" />
                <input
                  type="text"
                  name="village"
                  placeholder={signupCopy.villagePlaceholder}
            ...
            <div>
              <label className="mb-1 block text-sm font-semibold text-gray-700">{signupCopy.districtLabel}</label>
              <input
                type="text"
                name="district"
                placeholder={signupCopy.districtPlaceholder}
            ...
            <div>
              <label className="mb-1 block text-sm font-semibold text-gray-700">{signupCopy.stateLabel}</label>
              <input
                type="text"
                name="state"
                placeholder={signupCopy.statePlaceholder}
            ...
            <div className="md:col-span-2">
              <label className="mb-1 block text-sm font-semibold text-gray-700">{signupCopy.pincodeLabel}</label>
              <input
                type="text"
                name="pincode"
                placeholder={signupCopy.pincodePlaceholder}
            ...
            <div>
              <label className="mb-1 block text-sm font-semibold text-gray-700">{copy.login.emailLabel}</label>
              <div className="relative">
            ...
            <div>
              <label className="mb-1 block text-sm font-semibold text-gray-700">{copy.login.passwordLabel}</label>
              <div className="relative">
            ...
            <div className="md:col-span-2">
              <label className="mb-1 block text-sm font-semibold text-gray-700">{signupCopy.confirmPasswordLabel}</label>
              <div className="relative">
                <Lock className="absolute left-3 top-3.5 h-5 w-5 text-indigo-400" />
                <input
                  type={showConfirmPassword ? 'text' : 'password'}
                  name="confirmPassword"
                  placeholder={signupCopy.confirmPasswordPlaceholder}
